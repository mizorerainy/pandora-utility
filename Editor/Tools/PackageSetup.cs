...
namespace MizoreRainy.Pandora.Editor.Tools
{
    ...
    public class PandoraSettingsWindow : EditorWindow
    {
        ...
        /// When enabled, the ConfigLoader uses "config.yaml" instead of "config.txt".
        ...
        private void OnGUI()
        {
            ...
            if (!_IsYamlInstalled)
            {
                EditorGUILayout.HelpBox("Install the VYaml package to enable YAML configuration support.", MessageType.Info);
                if (GUILayout.Button("Install VYaml"))
                {
                    PackageSetup.InstallVYaml();
                }
            }

            EditorGUI.BeginDisabledGroup(!_IsYamlInstalled);
            EditorGUI.BeginChangeCheck();
            _UseYaml = EditorGUILayout.Toggle(new GUIContent("Use YAML Format (.yaml)", "If enabled, the ConfigLoader will use 'config.yaml' instead of 'config.txt'. This requires the VYaml package."), _UseYaml);
            if (EditorGUI.EndChangeCheck())
            {
    ...
    public static class PackageSetup
    {
        ...
        /// </summary>
        private const string _YAML_GIT_URL = "https://github.com/hadashiA/VYaml.git?path=VYaml.Unity/Assets/VYaml#0.27.1";
        ...
        /// The configuration associated with this key is stored and managed via <see cref="UnityEditor.EditorPrefs"/>.
        /// </summary>
        private const string _CONFIG_AUTO_INIT_KEY = "Pandora.ConfigLoader.AutoInitEnabled";
        ...
        /// stored in Unity's Editor Preferences, representing the state of YAML configuration usage.
        ...
        /// are added or removed as needed, allowing for conditional compilation in the Unity project.
        /// </summary>
        /// <remarks>
        ...
        /// of certain packages and reads editor preferences to determine the required definition symbols.
        ...
        public static void UpdateScriptingDefines()
        {
            ...
            var definesList = new HashSet<string>(definesString.Split(';').Where(_s => !string.IsNullOrEmpty(_s)).ToList());
            ...
            definesChanged |= SetDefine(ref definesList, _CONFIG_USE_YAML_SYMBOL, EditorPrefs.GetBool(_CONFIG_USE_YAML_KEY, false));
        ...
        private static bool SetDefine(ref HashSet<string> _defines, string _define, bool _shouldExist)
        {
            ...
            if (_shouldExist && !hasDefine) { _defines.Add(_define); return true; }
            if (!_shouldExist && hasDefine) { _defines.Remove(_define); return true; }
            return false;
        }
        ...
        /// If the user chooses to skip, the setup state will be marked as complete without performing the installation.
        /// </summary>
        private static void ShowSetupDialog()
        {
            ...
            var result = EditorUtility.DisplayDialogComplex("Pandora Network Utility Setup", message, "Install UniTask", "Skip (Install Manually)", "Cancel");
            switch (result)
            {
        ...
        private static void ShowCompletionDialog()
        {
            ...
            {
                EditorUtility.DisplayDialog("Setup Complete", "Pandora Package Setup Complete!\n\nAll required dependencies are installed.", "OK");
        ...
        private static async Task InstallPackageAsync(string _packageId, string _packageVersionOrUrl)
        {
            ...
            else if (request.Status >= StatusCode.Failure)
            {
                ...
                EditorUtility.DisplayDialog("Installation Failed", $"Failed to install package '{_packageId}'.\n\nError: {request.Error.message}\n\nPlease check the console for details.", "OK");
            }
        }
        ...
        internal static bool IsPackageInstalled(string _packageId)
        {
            ...
            if(request.Status == StatusCode.Success)
            {
                return request.Result.Any(_p => _p.name == _packageId);
            }
            return false;
        }
